...
export const useNotifications = (): UseNotifications => {
  ...
  const registerFriendNotificationReminder = async (contact: ContactModel) => {
    try {
      const notificationId =
        await notificationsService.registerNotificationForContact(
          contact,
          contact.lastCheckin ?? new Date(),
        );
      ...
      logService.error(
        `Unable to register notification for contact ${contact.id}`,
        e,
      );
    }
  };
  ...
  const registerFriendNotificationBirthdayReminder = async (
    contact: ContactModel,
  ) => {
    try {
      await notificationsService.registerBirthdayNotificationForContact(
        contact,
      );
    } catch (e) {
      logService.error(
        `Unable to register birthday notification for contact ${contact.id}`,
        e,
      );
    }
  };
  ...
  const postPoneReminder = async (contact: ContactModel, checkinDate: Date) => {
    if (!contact.id) {
      logService.error(
        `Unable to postpone notification for contact without id`,
      );
      return;
    }
    ...
    try {
      const newNotification =
        await notificationsService.deleteNotificationAndCreateNewPostponed(
      ...
      logService.error(
        `Unable to postpone notification for contact ${contact.id}`,
        e,
      );
    }
  };
  ...
  const deleteFriendNotification = async (contactId: ContactId) => {
    ...
    if (!notification) {
      logService.warn(
        "Notification not found for friend with contact id : " + contactId,
      );
      return;
    }
    notificationRepository.deleteReminder(notification.notification_id);
    await notificationsService.deleteNotificationWithId(
      notification.notification_id,
    );
  };
