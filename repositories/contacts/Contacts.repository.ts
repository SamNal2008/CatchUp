...
class LocalRepository implements ContactsRepository {
  ...
  public async addNewFriend(
    ...
    const res = this.db.runSync(
      `INSERT INTO ${LocalRepository.TABLE_NAME} (contact_id, frequency) VALUES (?, ?)`,
      [contactToSave.id, contactToSave.frequency],
    );
    if (res.changes <= 0) {
  ...
  public async update(contactToUpdate: ContactModel): Promise<ContactModel> {
    ...
    this.db.runAsync(
      `UPDATE ${LocalRepository.TABLE_NAME} SET frequency = ? WHERE contact_id = ?`,
      [contactToUpdate.frequency, contactToUpdate.id],
    );
  ...
  public async getById(contactId: ContactId): Promise<ContactModel | null> {
    ...
    return createNewContactEntity({
      ...
      birthDate: DateUtils.getBirthDateFromBirthday(contactInPhone),
      lastCheckin: null,
    });
  }
  ...
  public async getAll(): Promise<ContactModel[]> {
    ...
    const entities: ContactEntity[] = await this.db.getAllAsync<ContactEntity>(
      `SELECT * FROM ${LocalRepository.TABLE_NAME}`,
    );
    for (const entity of entities) {
      const contactInPhone: Contact.Contact | undefined =
      ...
      if (!contactInPhone) {
        logService.warn(
          `Contact with ID : ${entity.contact_id} not found in phone anymore`,
        );
        continue;
      }
      result.push(
