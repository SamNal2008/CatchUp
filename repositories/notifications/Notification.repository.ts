...
class LocalRepository implements NotificationRepository {
  ...
  public saveReminder(notificationEntity: NotificationEntity) {
    ...
    const res = this.db.runSync(
      `INSERT INTO ${LocalRepository.TABLE_NAME} (notification_id, contact_id, frequency) VALUES (?, ?, ?)`,
      [
        notificationEntity.notification_id,
        notificationEntity.contact_id,
        notificationEntity.frequency,
      ],
    );
    if (res.changes <= 0) {
      throw new Error(
        `Unable to save notification : ${notificationEntity.contact_id} - ${notificationEntity.notification_id}`,
      );
    }
  ...
  public getReminder(contactId: ContactId): NotificationEntity | null {
    logService.log(
      this.db.getFirstSync<NotificationEntity>(
        `SELECT * from ${LocalRepository.TABLE_NAME}`,
      ),
    );
    const res = this.db.getFirstSync<NotificationEntity>(
  ...
  public updateReminder(notificationEntity: NotificationEntity) {
    ...
    const res = this.db.runSync(
      `UPDATE ${LocalRepository.TABLE_NAME} SET notification_id = ?, frequency = ? where contact_id = ?`,
      [
        notificationEntity.notification_id,
        notificationEntity.frequency,
        notificationEntity.contact_id,
    ...
    if (res.changes <= 0) {
      throw new Error(
        `Unable to update notification : ${notificationEntity.contact_id} - ${notificationEntity.notification_id}`,
      );
    }
  }
  ...
  public deleteReminder(notificationId: NotificationId) {
    this.db.runSync(
      `DELETE from ${LocalRepository.TABLE_NAME} where notification_id = ?`,
      [notificationId],
    );
  }
