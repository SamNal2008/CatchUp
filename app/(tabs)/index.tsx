...
const CheckInMonthYear = ({
  checkInMonthWithYear,
  checkIns,
}: CheckInMonthYearProps) => {
  const month = DateUtils.getMonthName(checkInMonthWithYear);
  const year = checkInMonthWithYear.split(" ")[1];

  return (
    <View
      key={checkInMonthWithYear}
      style={{ width: Size.full, gap: Spacing.small }}
    >
      <ThemedText type={"sectionTitle"}>{`${month} ${year}`}</ThemedText>
      <FlatList
...
const CheckInSummary = ({ checkIn }: CheckInSummaryProps) => {
  const isNoteContentEmpty =
    checkIn?.noteContent === null || checkIn?.noteContent === "";
    ...
    >
      ...
      >
        ...
        {checkIn.contact?.image ? (
          ...
          <InitialImage
            firstName={checkIn.contact.firstName}
            lastName={checkIn.contact.lastName}
            size={32}
          />
        )}
      </View>
      {!isNoteContentEmpty ? (
        ...
        >
          <ThemedText style={{ fontFamily: "SF" }}>
            {checkIn.noteContent}
          </ThemedText>
        </View>
      ) : null}
...
export default function HomeScreen() {
  ...
  return (
    <ThemedView
      style={[
        styles.container,
        hasCheckIns ? styles.containerWithCheckin : null,
      ]}
    >
      {!hasCheckIns ? (
        <PlaceholderScreen />
