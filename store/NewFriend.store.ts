...
const newFriendStore = create<NewFriendStore>((set) => ({
    ...
    setContact: (contact) => {
        set({
            contact,
            contactBirthday: contact?.birthday ? DateUtils.getDateFromContactDate(contact.birthday) : null,
    ...
    selectedFrequency: "weekly",
    setSelectedFrequency: (selectedFrequency) => set({selectedFrequency: selectedFrequency ?? 'never'}),
    reset: () => set({
        contact: null,
        contactBirthday: null,
        contactLastCheckIn: new Date(),
